...
int	count_args(t_cmd *tmp)
{
	...
	count = 0;
	if (tmp->prev && tmp->prev->prev && tmp->prev->cmd[0] == '<'
		&& tmp->prev->prev->cmd[0] == '<' && tmp->prev->cmd[0] != '>')
		return (0);
	if (tmp->prev && (tmp->prev->cmd[0] == '>' || tmp->prev->cmd[0] == '<'))
		return (0);
	if (lst_len(tmp) == 1 && tmp->cmd[0] != '<' && tmp->cmd[0] != '>')
		return (1);
